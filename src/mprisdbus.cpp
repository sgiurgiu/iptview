...
MPRISDBus::MPRISDBus(QObject* parent) : QObject{ parent }
{
    ...
    QString serviceName =
        QString("org.mpris.MediaPlayer2.iptview.instance%1").arg(pid);
    if (!bus.registerService(serviceName))
    {
        // can't register service
        auto lastError = bus.lastError();
        if (lastError.type() == QDBusError::NoError)
            return;
        QString msg = QString("DBus register error: %1, %2, %3")
                          .arg(lastError.type())
                          .arg(lastError.name())
...
void MPRISDBus::propertyChanged(QString name, QVariantMap map, QStringList list)
{
    ...
    for (QVariantMap::const_iterator it = map.cbegin(), end = map.cend();
         it != end; ++it)
    {
...
void MPRISDBus::PlayingChannel(ChannelTreeItem* channel)
{
    if (!channel)
        return;
    metadata.clear();
    metadata["mpris:trackid"] =
        QDBusObjectPath(QString("/iptview/track/%1").arg(channel->getName()));
...
void MPRISDBus::SelectedChannel(ChannelTreeItem* channel)
{
...
{
    if (managerSignalingProperties.empty())
        return;
...
void MPRISDBus::notifyManagerChangedProperties(const QString& property,
                                               const QVariant& value)
{
    ...
    if (firstChange)
    {
        QMetaObject::invokeMethod(this,
                                  &MPRISDBus::emitManagerChangedPropertySignal,
                                  Qt::QueuedConnection);
    }
}
void MPRISDBus::emitPlayerChangedPropertySignal()
{
    if (playerSignalingProperties.empty())
        return;
    QDBusMessage signal = QDBusMessage::createSignal(
        "/org/mpris/MediaPlayer2",
        QStringLiteral("org.freedesktop.DBus.Properties"),
...
void MPRISDBus::notifyPlayerChangedProperties(const QString& property,
                                              const QVariant& value)
{
    ...
    {
        QMetaObject::invokeMethod(this,
                                  &MPRISDBus::emitPlayerChangedPropertySignal,
                                  Qt::QueuedConnection);
    }
}
...
DBusTracklist::GetTracksMetadata(const QList<QString>& trackIds) const
{
...
void DBusTracklist::AddTrack(const QString& uri,

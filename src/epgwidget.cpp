...
: QToolBar{ parent }, networkManager{ networkManager }
{
    ...
    connect(refreshListingsTimer, SIGNAL(timeout()), this,
            SLOT(retrieveEpgListings()));
    ...
    connect(previousListingAction, SIGNAL(triggered(bool)), this,
            SLOT(showPreviousListing()));
    connect(nextListingAction, SIGNAL(triggered(bool)), this,
            SLOT(showNextListing()));
...
void EPGWidget::SetChannel(ChannelTreeItem* channel)
{
    ClearChannel();
    ...
    if (selectedChannel)
    {
        QString xstreamServerTimezone =
            DatabaseProvider::GetDatabase()->GetXStreamServerTimezone(
                selectedChannel->getXStreamServerId());
        if (!xstreamServerTimezone.isEmpty())
        {
            ...
            if (serversTimezone.isValid())
            {
                ...
                serverTimezoneUTCDifference =
                    local.toTimeZone(serversTimezone).offsetFromUtc();
            }
        }
        retrieveEpgListings();
    }
...
void EPGWidget::buildEpgListings(const QJsonDocument& doc)
{
    ...
    for (const auto& listing : epgListingArray)
    {
        ...
        auto startTimestampString =
            listingObject.value("start_timestamp").toString("");
        auto endTimestampString =
            listingObject.value("stop_timestamp").toString("");
        if (startTimestampString.isEmpty() || endTimestampString.isEmpty())
            continue;
        bool ok = false;
        ...
        epgListing.title = QByteArray::fromBase64(titleEncoded.toUtf8(),
        ...
        epgListing.description = QByteArray::fromBase64(
            descriptionEncoded.toUtf8(), QByteArray::Base64Encoding);
...
void EPGWidget::displayCurrentListing()
{
    ...
    if (currentListingIndex >= 0 && currentListingIndex < listings.size())
    {
        ...
        auto title = QString("%1-%2 %3")
                         .arg(systemLocale.toString(listing.start.time(),
                                                    QLocale::ShortFormat),
                              systemLocale.toString(listing.end.time(),
                                                    QLocale::ShortFormat),
                              listing.title);
